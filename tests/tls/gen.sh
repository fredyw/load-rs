...
openssl req -new -x509 -key ca.key -out ca.crt -subj "/CN=Test CA" -extensions v3_ca -days 365
...
openssl x509 -req -in server.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out server.crt -extfile server.ext -days 365
...
openssl x509 -req -in client.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out client.crt -extfile client.ext -days 365
...
openssl req -new -x509 -key untrusted-ca.key -out untrusted-ca.crt -subj "/CN=Untrusted Test CA" -extensions v3_ca -days 365
...
openssl x509 -req -in untrusted-client.csr -CA untrusted-ca.crt -CAkey untrusted-ca.key -CAcreateserial -out untrusted-client.crt -extfile client.ext -days 365
